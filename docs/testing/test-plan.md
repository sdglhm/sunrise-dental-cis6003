...
The automated suite contains 14 JUnit tests and does not require a database server. The JDBC appointment DAO test uses an H2 in-memory database configured under `src/test/resources`, while the application remains configured for PostgreSQL outside test runs. The local PostgreSQL demonstration has also been manually checked for authentication, patient add/search/update, patient reuse during appointment creation, appointment save/find/update/cancel, filtered lists, catalogue operations, billing and reports.
