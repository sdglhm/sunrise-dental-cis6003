...
public class JdbcAppointmentDao implements AppointmentDao {
    ...
    public List<Appointment> findAll(LocalDate date, Long dentistId) {
        String sql=BASE+"WHERE (? IS NULL OR a.appointment_date=?) AND (? IS NULL OR a.dentist_id=?) ORDER BY a.appointment_date, a.appointment_time";
        try(Connection c=connection();PreparedStatement s=c.prepareStatement(sql)){if(date==null){s.setNull(1,Types.DATE);s.setNull(2,Types.DATE);}else{s.setDate(1,Date.valueOf(date));s.setDate(2,Date.valueOf(date));}if(dentistId==null){s.setNull(3,Types.BIGINT);s.setNull(4,Types.BIGINT);}else{s.setLong(3,dentistId);s.setLong(4,dentistId);}ResultSet r=s.executeQuery();List<Appointment> list=new ArrayList<>();while(r.next())list.add(map(r));return list;}catch(SQLException e){throw error(e);}
    }
