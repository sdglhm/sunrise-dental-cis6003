...
public class JdbcReportDao {
    ...
    private List<Map<String,Object>> query(String sql,String value) { try(Connection c=DatabaseConnectionFactory.getInstance().getConnection();PreparedStatement s=c.prepareStatement(sql)){if(value!=null)s.setString(1,value);ResultSet r=s.executeQuery();List<Map<String,Object>> rows=new ArrayList<>();ResultSetMetaData m=r.getMetaData();while(r.next()){Map<String,Object> row=new LinkedHashMap<>();for(int i=1;i<=m.getColumnCount();i++)row.put(m.getColumnLabel(i),r.getObject(i));rows.add(row);}return rows;}catch(SQLException e){throw new IllegalStateException("Could not create report",e);} }
}
